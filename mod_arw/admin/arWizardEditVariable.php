	...
	if($error!=1) {
		...
		if($hid_value > 1) {		 
			...
			if($tag=='Dropdown') {
				...
				$str="insert into variables(variable_name, variable_info,field,sequence_id,type) values('$variable_name','$variable_info','$make_field','$sequence_id','$tag')";
...
/*         if($variable_status == 'Select')
	 ...
	 $qry = mysql_query("update variables set variable_name='".$variable_name."',variable_info = '".$variable_info."', field = '".$field."', type = '".$variable_type. "',dropdown_values ='".$pick_all_up."' where sequence_id='".$_GET['sequence_id']."' and variable_id = '".$_GET['var_id']."'") or die(mysql_error());
		 }
		 else{
		 
	$qry = mysql_query("update variables set variable_name='".$variable_name."',variable_info = '".$variable_info."', field = '".$field."', type = '".$variable_type. "' where sequence_id='".$_GET['sequence_id']."' and variable_id = '".$_GET['var_id']."'") or die(mysql_error());
		 }
...
?>
<div class="grid_7" style="width:100%">
          <div class="box-header"> Manage Variable </div>
	...
	<?php echo $dropdown_contents;?></textarea>
		...
		<?php
			...
			<div class="row">
				...
				<a href="admin.php?page=arWizardEditVariableNow&sequence_id=<?php echo $_GET['sequence_id'];?>&var_id=<?php echo $auto_array['variable_id'];?>&<?php echo $page_no;?>">  
